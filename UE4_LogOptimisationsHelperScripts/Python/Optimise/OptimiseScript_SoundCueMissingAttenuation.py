...
with unreal.ScopedSlowTask(len(allAssets), selectedAssetsPath) as ST:
    ...
    for asset in allAssets:
        ...
        if _assetClassName == "SoundCue":
            ...
            _SCAttenuationName = _SoundCueAsset.get_editor_property("attenuation_settings")
            _SCAttenuationOverrideActive = _SoundCueAsset.get_editor_property("override_attenuation")

            if not SystemsLib.is_valid(_SCAttenuationName):  # check if asset plugged in
                if not _SCAttenuationOverrideActive:  # if not then check if override is set
...
# //////////////////////////////
TitleOfOptimisation = "Log Sound Cues With Missing Attenuation Assets"
DescOfOptimisation = "Searches the entire project for Sound Cues that have no attenuation plugged in and also no Attenuation override set to true"
SummaryMessageIntro = "-- Sound Cues With No Attenuation Settings --"
